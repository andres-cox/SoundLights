...
unsigned int timeLine[10][150] = {
  {0,  10, 32, 53, 61, 71, 82, 103, 112, 123, 133, 153, 163, 173,184, 203, 213, 223, 233, 253, 263, 272,281, 289, 300,
  308,318,321,326, 337,343,349, 356, 363, 375, 383, 393,396,401,412, 418,425,431,453, 487, 501, 519, 580, 618, 656, 658,660,662, 671 ,
  672, 674, 675, 683, 690, 693, 694, 699, 702, 704, 706, 708,710, 711, 712, 731, 745, 750, 761, 764, 766, 768, 768,771, 773 , 774,784 ,785,
  786, 787 , 792, 798, 806, 828, 862, 868, 875, 895 , 937 , 955 , 969, 993, 1031, 1050, 1068, 1076, 1087 , 1106, 1111 ,1125 , 1128, 1242},
  {0,10,30,48,65},
  {0,25,39,56,75}
};
...
void loop() {
  valPot = analogRead(A0);  // read the input pin
  ...
  if (musicState) {
    switch (var) {
      ...
      case 2:
        ...
        Serial.print("scene2: ");
        Serial.println(millis());
        drawScene(scene2, "DOWN", 150, "UP", 150, 0);
        ...
        Serial.print("drawfade: ");
        Serial.println(millis());
        drawFade(scene2, scene1, "UP", 150, "UP", 150, 2);

        Serial.print("blinkScenes: ");
        Serial.println(millis());
        blinkScenes(scene1, scene2, 150, 3);
...
void drawFade(Scene sceneStart, Scene sceneEnd, String elevator_1, int elevatorSpeed_1, String elevator_2, int elevatorSpeed_2, int t)
{
  ...
  unsigned long  fade;
  unsigned int fadeStart = timeLine[var - 1][t];// + 100;
